...
def load(
    ...
    if dynamic_batching:
        from baichuan.modeling.dynamic_batching.Model import TensorDumper, Transformer
        from baichuan.modeling.dynamic_batching.Pipeline import BaiChuan
        if cache_layout != 3:
            print("Info: we suggest using cache_layout 3 for cuda inference performance")
    else:
        from baichuan.modeling.static_batching.Model import TensorDumper, Transformer
        from baichuan.modeling.static_batching.Pipeline import BaiChuan
        if cache_mode:
