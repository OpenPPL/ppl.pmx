...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor],
        ...
        TensorDumper.dump(x, "layer{}_x".format(self.layer_id))
        if self.fused_qkv:
            xqkv = self.wqkv(x)
            TensorDumper.dump(self.wqkv.weight, "layer{}_wqkv_weight".format(self.layer_id))
            TensorDumper.dump(self.wqkv.bias, "layer{}_wqkv_bias".format(self.layer_id))
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, skip: torch.Tensor, attn_mask: Optional[torch.Tensor],
        ...
        norm, res1 = self.attention_norm(x, skip)
        ...
        # TensorDumper.dump(x, "layer{}_attention".format(self.layer_id))
        norm, res2 = self.ffn_norm(attn, res1)
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, attn_mask: Optional[torch.Tensor],
                start_pos: torch.Tensor, pos_idx: torch.Tensor, kv_cache: torch.Tensor, kv_scale: torch.Tensor = None):
