...
class BigCode(__TextGenerator__):
    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_size, total_len):
            ...
            pos_idx = torch.arange(prev_pos, cur_pos).to(token_ids.device)
            ...
            logits = self.model.forward(token_ids, attn_mask, start_pos, pos_idx, kv_cache, kv_scale)
            if temperature > 0:
    ...
    def export(
        ...
        attn_mask = torch.empty(0, dtype=torch.float16)
        pos_idx = torch.ones(total_len // 2, dtype=torch.int64)
        # import ipdb;ipdb.set_trace()

        input_names = ["token_ids", "attn_mask", "start_pos", "pos_idx", "kv_cache", "kv_scale"]
        output_names = ["logits"]
        ...
        torch.onnx.export(
            ...
            (tokens_ids, attn_mask,
             start_pos, pos_idx, kv_cache, kv_scale),
