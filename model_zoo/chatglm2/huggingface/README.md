...
Download the model file from the [Hugging Face](https://huggingface.co/THUDM/chatglm2-6b).
...
```bash
OMP_NUM_THREADS=1 torchrun --nproc_per_node $num_gpu Demo.py --ckpt_dir <chatglm2_dir> --tokenizer_path <chatglm2_tokenizer_dir>/tokenizer.model --fused_qkv 1 --fused_kvcache 1 --auto_causal 1 --quantized_cache 1 --dynamic_batching 1
```
...
```bash
OMP_NUM_THREADS=1 torchrun --nproc_per_node $num_gpu Export.py --ckpt_dir <chatglm2_dir> --tokenizer_path <chatglm2_tokenizer_dir>/tokenizer.model --fused_qkv 1 --fused_kvcache 1 --auto_causal 1 --quantized_cache 1 --dynamic_batching 1 --export_path <export_dir>
```
...
```bash
OMP_NUM_THREADS=1 torchrun --nproc_per_node $num_gpu Demo.py --ckpt_dir <chatglm2_dir> --tokenizer_path <chatglm2_tokenizer_dir>/tokenizer.model --fused_qkv 1 --fused_kvcache 1 --auto_causal 1 --quantized_cache 1 --dynamic_batching 1 --seqlen_scale_up 1 --max_gen_len 256 --dump_steps 0,1,255 --dump_tensor_path <dump_dir>  --batch 1
```
...
Make sure to replace `<chatglm2_dir>` , `<chatglm2_tokenizer_dir>` and `<dump_tensor_path>`with the actual directory paths in your environment.
