...
def load(
    ...
    if dynamic_batching:
        from falcon.modeling.dynamic_batching.Model import TensorDumper, Transformer
        from falcon.modeling.dynamic_batching.Pipeline import Falcon
        if cache_layout != 3:
            print("Info: we suggest using cache_layout 3 for cuda inference performance")
    else:
        from falcon.modeling.static_batching.Model import TensorDumper, Transformer
        from falcon.modeling.static_batching.Pipeline import Falcon
        if cache_mode:
