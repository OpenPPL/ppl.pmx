...
def write_pmx_model(model_path, input_base_path):
    ...
    for layer_i in range(pmx_params_dict['num_layers']):
        ...
        tmp_wq = torch.zeros(size=[(num_heads+1) * dims_per_head, wq.shape[1]], dtype=wq.dtype)
        tmp_wk = torch.zeros(size=[(num_kv_heads+1) * dims_per_head, wk.shape[1]], dtype=wk.dtype)
        tmp_wv = torch.zeros(size=[(num_kv_heads+1) * dims_per_head, wv.shape[1]], dtype=wv.dtype)
        tmp_wq[:wq.shape[0], :] = wq
        tmp_wk[:wk.shape[0], :] = wk
        tmp_wv[:wv.shape[0], :] = wv

        tmp_wo = torch.zeros(size=[wq.shape[0], (num_heads+1) * dims_per_head], dtype=wq.dtype)
        ...
        tmp_q_norm_w = torch.zeros(size=[(num_heads+1) * dims_per_head], dtype=wq.dtype)
        tmp_k_norm_w = torch.zeros(size=[(num_kv_heads+1) * dims_per_head], dtype=wq.dtype)
