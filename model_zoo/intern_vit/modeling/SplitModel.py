...
def split_pmx_model(model_path, input_base_path, num_shards):
    ...
    for layer_i in range(params['num_layers']):
        ...
        wo = state_dict[f"layers.{layer_i}.attention.wo.weight"].split([dims_per_head * params['num_heads'] // num_shards]*num_shards, dim=1) #due to pad wqkv
        ...
        q_norm_w = state_dict[f"layers.{layer_i}.attention.q_norm.weight"].split([dims_per_head * params['num_heads'] // num_shards]*num_shards, dim=0)
