...
def split_pmx_model(model_path, input_base_path, num_shards):
    ...
    for layer_i in range(params['num_layers']):
        ...
        assert state_dict[f"layers.{layer_i}.attention.wq.weight"].shape[0] // (dims_per_head * num_shards) > 1
        ...
        assert state_dict[f"layers.{layer_i}.attention.wk.weight"].shape[0] // (dims_per_head * num_shards) > 1
        ...
        assert state_dict[f"layers.{layer_i}.attention.wv.weight"].shape[0] // (dims_per_head * num_shards) > 1
