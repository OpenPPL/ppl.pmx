...
def load(
    ...
    if dynamic_batching:
        from mistral.modeling.dynamic_batching.Model import TensorDumper, Transformer
        from mistral.modeling.dynamic_batching.Pipeline import Mistral
        if cache_layout != 3:
            print("Info: we suggest using cache_layout 3 for cuda inference performance")
    else:
        from mistral.modeling.static_batching.Model import TensorDumper, Transformer
        from mistral.modeling.static_batching.Pipeline import Mistral
        if cache_mode:
